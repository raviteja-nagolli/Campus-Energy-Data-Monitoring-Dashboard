...
export default function EnergyDashboard() {
  ...
  const fetchEnergyData = async () => {
    try {
      ...
      // const activeAlertBuildings =
      //   new Set(
      //     alertsData.map(a => a.msg.split(" → ")[0])
      //   ).size;

      // setEfficiency(
      //   Math.max(
      //     0,
      //     Math.round(100 - activeAlertBuildings * 5)
      //   )
      // );

      const problematicSensors = sensorData.filter(
        s =>
          s.current > s.maxThreshold ||
          s.current < s.minThreshold
      ).length;
      ...
      const efficiency =
        totalSensors === 0
          ? 100
          : Math.round(
            ((totalSensors - problematicSensors)
              / totalSensors) * 100
          );
